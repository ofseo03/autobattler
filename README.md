...
- 배치: 마우스 드래그로 칸 스왑 / 벤치 ↔ 라인업 교체
- 키보드: 방향키로 칸 이동, `Space` 로 집기/놓기
- 경기 로그: `Space` 또는 "빠르게" 버튼으로 즉시 전체 출력
- `ESC`: 패닉 모드 (평범한 문서 화면으로 전환, 다시 누르면 복귀. 상태 유지)
...
엔진만 따로 검증하려면 Node 가 필요하다 (제품에는 필요 없음):
...
- **시너지 효과**: 같은 로스터를 시너지 없이 돌렸을 때 대비 득점 **×1.51**, 시너지를 노려 영입·배치한 팀 vs 무작위 팀 **×1.24**. 목표 1.5~2배에 맞추려 프롬프트 초기값에서 런(12/20/30→18/28/40%)·세트(8/18/30→10/20/32%)를 올렸고, 정확도→타율 곡선을 볼록(`hitExp 1.5`)하게 바꿔 높은 능력치 1점의 가치가 크도록 했다. 클린업 트리오는 240 으로는 한 번도 발동하지 않아(카드 장타력 평균 55~70) 200 으로 내렸다.
- **라인업 변경**: 시즌당 평균 **1.3~2.2칸** (목표 2~3). 노화 표와 23세 이하 자연 성장이 주된 압력.
- **난이도**: 시너지를 노린 팀은 커리어당 우승 **2.9회**(시즌 승률 .54), 무작위 팀은 **1.4회**. 첫 조정본(우승 1.2회, 승률 .47)은 플레이해 보니 너무 어려워 상대 득점(`oppRunsBase` 3.1→2.95), 시작 예산(+4), 명성→예산(`budgetPerFame` 1/6→1/5), 리그 편차(`otherTeamSd` .075→.065)를 완화했다. 더 어렵게 하려면 `oppRunsBase` 를 올리는 것이 가장 직접적이다. 우승→명성→예산의 눈덩이를 막으려 상대 전력이 명성에 약하게 연동(`fameStrength`)된다.
